...
1. Author the trap under `tests/autogenerated/`.
2. Compile and run against the **unmodified** engine:
   ```bash
   clang++ -std=c++20 -Isrc/engine src/engine/text_shaper.cpp <test_file>.cpp -o /tmp/gate_a_trap && /tmp/gate_a_trap
   ```
3. **Receipt Requirement**: Confirm non-zero exit and record failure diagnostic.

### Gate B: Implementation Resolution
1. Apply engine fix.
2. Recompile and run:
   ```bash
   clang++ -std=c++20 -Isrc/engine src/engine/text_shaper.cpp <test_file>.cpp -o /tmp/gate_b_verify && /tmp/gate_b_verify
   ```
