...
| **The Acid Pit** | CI / Runtime Execution Tool | Runtime autopsy & memory validation. Automatically orchestrates dedicated multi-pass sanitizer builds (AddressSanitizer, MemorySanitizer, ThreadSanitizer, UBSan) using the host build system (e.g. GN/Ninja, CMake). Executes test binaries under instrumented memory to catch leaks, use-after-free, uninitialized reads, or alignment faults before code is accepted. |
